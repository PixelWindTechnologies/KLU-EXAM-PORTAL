...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(prev => {
        if (prev <= 1) { clearInterval(timerRef.current); doSubmit('time_expired'); return 0; }
        return prev - 1;
      });
    }, 1000);
  ...
  const doSubmit = useCallback(async (type = 'manual') => {
    ...
    const answersArray = Object.entries(answers).map(([qId, sel]) => ({
      questionId: parseInt(qId), selectedOption: sel
    }));
    try {
      await examAPI.submit({ answers: answersArray, tabSwitchCount: tabSwitchRef.current, cheatingWarnings: warningRef.current, submissionType: type });
    } catch {}
    navigate('/result');
  }, [answers, navigate]);
    ...
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'white' }}>
        <div className="card" style={{ maxWidth: 440, textAlign: 'center' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Your exam was terminated due to {warningRef.current} policy violations. Submitting your responses...</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: 'white', userSelect: 'none' }}>
      ...
      <div style={{ height: 56, background: 'white', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 20px', boxShadow: 'var(--shadow-sm)', flexShrink: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.9rem', color: 'var(--text-primary)' }}>PixelWind <span style={{ color: 'var(--accent)' }}>SecureAssess</span></span>
        </div>

        {/* Timer */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 18px', borderRadius: 8, background: isCritical ? 'var(--red-light)' : isUrgent ? 'var(--yellow-light)' : 'var(--bg-secondary)', border: `1.5px solid ${isCritical ? '#fecaca' : isUrgent ? '#fde68a' : 'var(--border)'}` }}>
          <FiClock size={16} color={isCritical ? 'var(--red)' : isUrgent ? 'var(--yellow)' : 'var(--accent)'} />
          <span style={{ fontFamily: 'monospace', fontWeight: 800, fontSize: '1.3rem', color: isCritical ? 'var(--red)' : isUrgent ? 'var(--yellow)' : 'var(--text-primary)', animation: isCritical ? 'blink 1s infinite' : 'none' }}>
            {formatTime(timeLeft)}
          </span>
        </div>
      ...
      {/* Main layout: Question left, Sidebar right */}
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '28px 36px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
            ...
            <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>Question {currentIndex + 1} of {questions.length}</span>
          </div>

          {/* Question Text */}
          <h2 style={{ fontSize: '1.05rem', fontWeight: 600, color: 'var(--text-primary)', lineHeight: 1.7, marginBottom: 28, textAlign: 'left' }}>
            {q?.question}
          </h2>
              ...
              return (
                <button key={idx} onClick={() => setAnswers({ ...answers, [q.id]: idx })}
                  style={{
                    ...
                    fontWeight: isSelected ? 600 : 400, boxShadow: isSelected ? '0 0 0 3px rgba(26,86,219,0.08)' : 'none'
                  }}>
        ...
        <div style={{ display: 'flex', flexShrink: 0 }}>
          {/* Toggle Button */}
          <div style={{ width: 28, background: 'var(--bg-secondary)', borderLeft: '1px solid var(--border)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: 20, cursor: 'pointer' }} onClick={() => setSidebarOpen(!sidebarOpen)}>
            <button style={{ background: 'white', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 5px', cursor: 'pointer', display: 'flex', color: 'var(--text-secondary)', boxShadow: 'var(--shadow-sm)' }} title={sidebarOpen ? 'Close panel' : 'Open panel'}>
            ...
            <div style={{ width: 240, background: 'var(--bg-secondary)', borderLeft: '1px solid var(--border)', overflowY: 'auto', padding: '16px 14px' }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 14, fontSize: '0.72rem', color: 'var(--text-secondary)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  <div style={{ width: 12, height: 12, borderRadius: 3, background: 'var(--green)', border: '1px solid var(--green)' }} />Answered
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                ...
                {questions.map((qs, i) => {
                  ...
                  return (
                    <button key={i} onClick={() => setCurrentIndex(i)}
                      style={{
                        aspectRatio: '1', border: `2px solid ${isCur ? 'var(--accent)' : isAns ? 'var(--green)' : 'var(--red)'}`,
                        borderRadius: 6,
